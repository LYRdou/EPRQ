#include<iostream>
#include<vector>
#include<queue>
#include<math.h>
#include<ctime>
#include<random>
#include<sstream>
#include<windows.h>
#define null INT_MIN
using namespace std;




struct TreeNode
{
    int val;
    TreeNode *left, *right;
    TreeNode(int x) : val(x), left(nullptr), right(nullptr) {}
};
 

TreeNode* buildTree(vector<int>& nums)
{
	if (nums.empty()) return nullptr;
    TreeNode *root = new TreeNode(nums.front());
    vector<TreeNode*> q = {root};
    int i = 1;
    while(!q.empty() && i < nums.size())
    {
        TreeNode *cur = q.front();
        q.assign(q.begin() + 1, q.end());
        if(i < nums.size())
        {
            cur->left = new TreeNode(nums[i++]);
            q.push_back(cur->left);
        }
        if(i < nums.size())
        {
            cur->right = new TreeNode(nums[i++]);
            q.push_back(cur->right);
        }
    }
    return root;
}


const unsigned char SBox[16][16] = {
    
};


const unsigned int Rcon[10] = {
    0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
    0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000
};


unsigned int SubWord(unsigned int X){
    unsigned char a3 =  X & 0x000000ff;
    unsigned char a2 = (X & 0x0000ff00) >> 8;
    unsigned char a1 = (X & 0x00ff0000) >> 16;
    unsigned char a0 = (X & 0xff000000) >> 24;
    unsigned char b3 = SBox[(a3 & 0xf0) >> 4][a3 & 0x0f];
    unsigned char b2 = SBox[(a2 & 0xf0) >> 4][a2 & 0x0f];
    unsigned char b1 = SBox[(a1 & 0xf0) >> 4][a1 & 0x0f];
    unsigned char b0 = SBox[(a0 & 0xf0) >> 4][a0 & 0x0f];

    return ((b0 << 24) | (b1 << 16) | (b2 << 8) | (b3));
}


void ADDROUNDKEY(unsigned int *state, unsigned int *w, int num_round){
    state[0] = state[0] ^ w[4 * num_round + 0];
    state[1] = state[1] ^ w[4 * num_round + 1];
    state[2] = state[2] ^ w[4 * num_round + 2];
    state[3] = state[3] ^ w[4 * num_round + 3];
}


void SUBBYTES(unsigned int *state){
    state[0] = SubWord(state[0]);
    state[1] = SubWord(state[1]);
    state[2] = SubWord(state[2]);
    state[3] = SubWord(state[3]);
}


unsigned int RotRows(unsigned int A, int ROW){
    return (A >> (sizeof(unsigned int)*8 - 8*ROW) | (A << 8*ROW));
}


unsigned int RotWord(unsigned int A){
    return (A >> (sizeof(unsigned int)*8 - 8) | (A << 8));
}

void SHIFTROWS(unsigned int *state){
    unsigned int b0 = (state[0] & 0xff000000) | ((state[1] & 0xff000000) >> 8) | ((state[2] & 0xff000000) >> 16) | ((state[3] & 0xff000000) >> 24);
    unsigned int b1 = ((state[0] & 0x00ff0000) << 8) | (state[1] & 0x00ff0000) | ((state[2] & 0x00ff0000) >> 8 ) | ((state[3] & 0x00ff0000) >> 16);
    unsigned int b2 = ((state[0] & 0x0000ff00) << 16) | ((state[1] & 0x0000ff00) << 8) | (state[2] & 0x0000ff00) | ((state[3] & 0x0000ff00) >> 8 );
    unsigned int b3 = ((state[0] & 0x000000ff) << 24) | ((state[1] & 0x000000ff) << 16) | ((state[2] & 0x000000ff) << 8) | (state[3] & 0x000000ff);
    b0 = RotRows(b0, 0);
    b1 = RotRows(b1, 1);
    b2 = RotRows(b2, 2);
    b3 = RotRows(b3, 3);
    state[0] = (b0 & 0xff000000) | ((b1 & 0xff000000) >> 8) | ((b2 & 0xff000000) >> 16) | ((b3 & 0xff000000) >> 24);
    state[1] = ((b0 & 0x00ff0000) << 8) | (b1 & 0x00ff0000) | ((b2 & 0x00ff0000) >> 8 ) | ((b3 & 0x00ff0000) >> 16);
    state[2] = ((b0 & 0x0000ff00) << 16) | ((b1 & 0x0000ff00) << 8) | (b2 & 0x0000ff00) | ((b3 & 0x0000ff00) >> 8 );
    state[3] = ((b0 & 0x000000ff) << 24) | ((b1 & 0x000000ff) << 16) | ((b2 & 0x000000ff) << 8) | (b3 & 0x000000ff);
}


unsigned char GFMul2(unsigned char x){
    if((x & 0x80) == 0x00)
        return x << 1;
    else
        return ((x << 1) ^ 0x1b);
}


unsigned char GFMul3(unsigned char x){
    return (x^GFMul2(x));
}


void MIXCOLUMNS(unsigned int *state){
    for(int i = 0; i < 4; i++){
        unsigned char a0 = (state[i] & 0xff000000) >> 24;
        unsigned char a1 = (state[i] & 0x00ff0000) >> 16;
        unsigned char a2 = (state[i] & 0x0000ff00) >> 8 ;
        unsigned char a3 = (state[i] & 0xff0000ff);
        unsigned char b0 =  GFMul2(a0)^GFMul3(a1)^a2^a3;
        unsigned char b1 =  a0^GFMul2(a1)^GFMul3(a2)^a3;
        unsigned char b2 =  a0^a1^GFMul2(a2)^GFMul3(a3);
        unsigned char b3 =  GFMul3(a0)^a1^a2^GFMul2(a3);

        state[i] = (b0 << 24) | (b1 << 16) | (b2<< 8) | b3;
    }
}


void KeyExpansion(unsigned int *key, unsigned int *w, unsigned int Nk, unsigned int Nr){
    unsigned int i = 0;
    while(i <= Nk-1){
        w[i] = key[i];
        i = i + 1;
    }
    while(i <= 4*Nr + 3){
        unsigned int temp = w[i-1];
        if(i % Nk == 0)
            temp = SubWord(RotWord(temp))^Rcon[i/Nk-1];
        else if(Nk > 6 && i%Nk == 4)
            temp = SubWord(temp);
        w[i] = w[i-Nk]^temp;
        i = i + 1;
        
    }
}

int aes(int num, int key = 0x09cf4f3c){
    unsigned int input[4] = {0, 0, 0, 0};
    input[3] = num;
    unsigned int KEY[4]   = {0x2b7e1516, 0x28aed2a6, 0xabf71588, 0x09cf4f3c};
    KEY[4] = key;
    unsigned int Nk = 4;
    unsigned int Nr = 10;

    unsigned int w[4*(Nr+1)] = {0x00000000};
    KeyExpansion(KEY, w, Nk, Nr);
    ADDROUNDKEY(input, w, 0);
    for(int i = 1; i <= (Nr - 1); i++){
        SUBBYTES(input);
        SHIFTROWS(input);
        MIXCOLUMNS(input);
        ADDROUNDKEY(input, w, i);
    }
    SUBBYTES(input);
    SHIFTROWS(input);
    ADDROUNDKEY(input, w, Nr);
    int res = input[0]+input[1]+input[2]+input[3];
    return res;
}


string IntToString(int & i)
{
  string s;
  stringstream ss(s);
  ss<<i;
  return ss.str();
}


string Binary(int num, int MAX){
	int bin[MAX], length = 0, value[MAX];
	int m_num = num, i = 0;
    string strs;
    for(int i = 0; i<MAX; i++){
        bin[i] = 0;
    }
	do
	{
		bin[i] = num % 2;
		if(num == 0 || num == 1){
			num = 1;
		}
		else{
			num = num / 2;
			i++;
			length++;
		}

	} while (num != 1);
	if(m_num != 0){
		bin[length] = num;
		length++;
		}
    for(int i = 0; i<MAX; i++){
        value[i] = bin[MAX-i-1];
    }
	for(int i=0;i<MAX;i++)
	{
		int &temp=value[i];
		strs+=IntToString(temp);
	}
	return strs;
}


int * index_F(int MAX, int p[], int *index_b, int s, int m1){
    int x = MAX/s;
    int t = pow(2, s);

    for(int i = 0; i < x*(2*t-1) + m1; i++){
		if(*(index_b+i) == 0){
			p[i] = aes(0+i);
		}else if(*(index_b+i) == 1){
			p[i] = aes(1+i);
		}
	}
    return p;
}



int * index_F_2(int MAX, int p[], int index[], int s, int m1, int no){
    int x = MAX/s;
    int t = pow(2, s);  

    for(int i = 0; i < x*(2*t-1)+m1; i++){
		if(index[i] == 0){
			p[i] = aes(0+i)+no;
		}else if(index[i] == 1){
			p[i] = aes(1+i)+no;
		}
	}
    return p;
}


int * index_num(int num, int MAX, int index[], int s, int m1){
    int count = 1;
    string i_num = Binary(num, MAX);
    int x = MAX/s;
    int t = pow(2, s);
    int index_s[2*t-1];
    int n = 0;
    for(int i = 0; i<x; i++){
        memset(index_s, 0, sizeof(index_s));
        for(int j = n; j<(i+1)*s; j++){
            index_s[count-1] = 1;
            if(i_num[j] == '0'){
                count = 2*count;
                index_s[count-1] = 1;
            }
            else{
                count = 2*count+1;
                index_s[count-1] = 1;
            }
        }
        count = 1;
        n = n+s;
        memcpy(&(index[i*(2*t-1)]), index_s, sizeof(index_s));
    }

    for(int i = 1; i<m1+1; i++){
        index[x*(2*t-1) + i] = i%2;
    }
    return index;
}




int * trapdoor_F(int left, int right, int MAX, int key_trap, int S[], string bin_1[], int &d0, int &d1, int s, int m1){
    int x = MAX/s;
    int t = pow(2, s);  
    int m_min = 0;
	int m_max = pow(2, MAX)-1;
    int m_temp = 0;
    int t_count = 0;
    int num_encryt = 0;
    int count = 1;
    if((m_max>=left&&m_max<=right) || (m_min<=left&&m_max>=right) || (m_min>=left&&m_min<=right)){
        if(right%2 == 0){
            m_temp = right-1;}
        else{
            m_temp = right;
            }
        if((m_temp-left+1)%2 == 1){
            string strs_l = Binary(left, MAX);
            bin_1[0] = strs_l;
            for(int i = 1; i<(m_temp-left)/2+1; i++){
                int temp = left+ 2*i-1;
                string strs = Binary(temp, MAX);
                strs[MAX-1] = '*';
                bin_1[i] = strs;
                t_count = i;
            }
            if(right%2 ==0){
                string strs_r = Binary(right, MAX);
                t_count = t_count+1;
                bin_1[t_count] = strs_r;
            }
        }else if((m_temp-left+1)%2 == 0){
            for(int i = 0; i<(m_temp-left)/2+1; i++){
                int temp = left+ 2*i;
                string strs = Binary(temp, MAX);
                strs[MAX-1] = '*';
                bin_1[i] = strs;
                t_count = i;
            }
            if(right%2 == 0){
                string strs_r = Binary(right, MAX);
                t_count = t_count + 1;
                bin_1[t_count] = strs_r;
            }
        }
    }


    for(int i = 1; i<MAX+1; i++){
        int t = pow(2,(i-1));
        for(int j = 0; j < m_max/2-1; j++){
            if((bin_1[j][MAX-i] == '*')&&(bin_1[j][MAX-i-1] == '0')&&(bin_1[j+t][MAX-i] == '*')){
                for(int k = j+1; k < m_max/2; k++){
                    if(bin_1[k][0] != 'c'){
                        bin_1[j][MAX-i-1] = '*';
                        bin_1[j+t] = "clear";}
                    }
            }
        }
    }

    int lon = 0;

    string ss_trapdoor(x*(2*t-1)+m1, '*');
    int n = 0;

    for(int i = 0; i<x; i++){
        string ss_trapdoor_s(2*t-1, '*');
        for(int j = 0; j<m_max/2; j++){
            ss_trapdoor_s[count-1] = '1';
            for(int k = n; k<(i+1)*s; k++){
                if(bin_1[j][k] == '0'){
                    count = 2*count;
                    ss_trapdoor_s[count-1] = '1';
                }else if((bin_1[j][k] == '1') && (bin_1[j][0] != 'c')){
                    count = 2*count+1;
                    ss_trapdoor_s[count-1] = '1';
                }else if((bin_1[j][k] == '*') && (bin_1[j][0] != 'c')){
                    count = 2*count;
                    lon = lon +1;
                    for(int w = 0; w<pow(2, lon); w++){
                        ss_trapdoor_s[count-1+w] = '*';
                    }
                }
                
            }
            lon = 0;
            count = 1;   
        }
        n = n+s;
        strncpy(&(ss_trapdoor[i*(2*t-1)]), &(ss_trapdoor_s[0]),2*t-1);
    }


    int size = 0;
    int fin_j = 0;
    for(int i = 0, j = 0; i<x*(2*t-1)+m1; i++){
        if((ss_trapdoor[i]=='1')){
            num_encryt = num_encryt+aes(1+i);
            S[j] = i;
            size = size + sizeof(S[j]);
            j++;
            fin_j = j;
        }
    }

    int loc = rand()%m1;

    S[fin_j] = loc + x*(2*t-1);
    int last = loc%2;
    
    num_encryt = num_encryt+aes(last + loc + x*(2*t-1));

    d0 = num_encryt + key_trap;
    d1 = aes(0, key_trap);
    return S;
}



string query_F(int MAX, int num_encryt, int *trapdoor_S, int d0, int d1, int *index_p, int s, int m1, int no, int number){
    int t = pow(2, s);
    int x = MAX/s;
    for(int i = 0; i<x*(2*t-1)+m1; i++){
        if(*(trapdoor_S+i) == -1){
            break;
        }
        int n = *(trapdoor_S+i);
        if(no <= number-2){
            num_encryt = num_encryt + *(index_p+n) - no;
        }
        else{
            num_encryt = num_encryt + *(index_p+n);
        }
    }

    int key_not = d0 - num_encryt;
	int key_0 = aes(0, key_not);
    std::ios::sync_with_stdio(false);
	if(key_0 == d1){
        return "c";
	}else{
        return "noc";
    }
}


void preOrderTraversal(TreeNode* root, int *index_p[], int *trapdoor_S, int d0, int d1, int MAX, int num_encryt, int number, int index_number[], int s, int m1) {
    if (root == nullptr) {
	        return;
    }
    int i = root->val-1;

    string res = query_F(MAX, num_encryt, trapdoor_S, d0, d1, index_p[i], s, m1, i, number);
    if(res == "c"){
        preOrderTraversal(root->left, index_p, trapdoor_S, d0, d1, MAX, num_encryt, number, index_number, s, m1);
        preOrderTraversal(root->right, index_p, trapdoor_S, d0, d1, MAX, num_encryt, number, index_number, s, m1);
    }

}

 
int main()
{
    int MAX = 8; //string len
    int s = 4;
    int x = MAX/s;
    int t = pow(2, s);  
    int tMAX = pow(2, MAX); 
    int m_max = pow(2, MAX)-1;
    int number = 8;
    int m1 = 2;
    int index_number[number];
    double level = log2(number)+1;
    int *index_p[2*number-1];
    int *index_b[2*number-1];
    vector<int> nums;
    for (int i = 0; i < 2*number-1; i++)
    	nums.push_back(i+1);
    TreeNode *root = buildTree(nums);
    
    int cou1 = number;
    int cou2 = number-1;
    int index[2*number-1][x*(2*t-1)+m1];
    for(int i = 0; i<2*number-1; i++){
        fill(index[i], index[i]+x*(2*t-1)+m1,0);
    }
    
    int p[2*number-1][x*(2*t-1)+m1];
    int p_nor[2*number-1][x*(2*t-1)+m1];
    default_random_engine e;
    uniform_int_distribution<int> u(0,tMAX-1);
    int num = 0;
    e.seed(time(0));




    std::ios::sync_with_stdio(false);
    for(int j = 0; j<level; j++){
        for(int i = cou2; i<cou1+cou2; i++){
            if(i>number-2){
                num++;
                num = num%m_max;
                index_number[i-number+1] = num;
                index_b[i] = index_num(num, MAX, index[i], s, m1);
                index_p[i] = index_F(MAX, p[i], index_b[i], s, m1);
                for(int k = 0; k<x*(2*t-1)+m1; k++){
                    index[i][k] = *(index_b[i]+k);
                }
            }
            else{
                for(int k = 0; k<x*(2*t-1)+m1; k++){
                    index[i][k] = (index[2*i+1][k]||index[2*i+2][k]);
                }
                int no = i;
                index_p[i] = index_F_2(MAX, p[i], index[i], s, m1, no);
            }
        }
        cou2=cou2/2;
        cou1=cou1/2;
    }


    int left = 0;
    int right = 1; //range
    int m_temp = 0;
    int t_count = 0;
    int num_encryt = 0;
    int S[x*(2*t-1)+m1];
    fill(S,S+x*(2*t-1)+m1,-1);
    string bin_1[m_max/2];
    for(int i = 0; i<m_max/2; i++){
        bin_1[i] = "clear";
    }


    int key_trap = 20;
    int d0;
    int d1;
    int *trapdoor_S = trapdoor_F(left, right, MAX, key_trap, S, bin_1, d0, d1, s, m1);

    
    std::ios::sync_with_stdio(false);
    preOrderTraversal(root, index_p, trapdoor_S, d0, d1, MAX, num_encryt, number, index_number, s, m1);

    return 0;
}